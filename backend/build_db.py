...
def insert_questions(conn, questions):
    ...
    for q in questions:
        ...
        group_id    = str(uuid.uuid4())  # Common group_id for both variants

        c.execute(
            "INSERT OR IGNORE INTO questions (id, chapter_id, text, type, group_id) VALUES (?,?,?,?,?)",
            (qid_closed, q["chapter_id"], q["text"], "closed", group_id)
        )
        c.execute(
            "INSERT OR IGNORE INTO questions (id, chapter_id, text, type, group_id) VALUES (?,?,?,?,?)",
            (qid_open, q["chapter_id"], q["text"], "open", group_id)
        )
