...
def get_edge_config():
    ...
    return {
        'id': edge_config_id,
        'token': edge_config_token
    }

def read_edge_config(key):
    config = get_edge_config()
    
    # Use Vercel's Edge Config API
    headers = {}
    if config['token']:
        headers['Authorization'] = f'Bearer {config["token"]}'
    
    url = f'https://api.vercel.com/v1/edge-config/{config["id"]}/item/{key}'
    ...
    try:
        ...
        print(f"Failed to read from Edge Config: {e}")
        return None

def write_edge_config(key, value):
    config = get_edge_config()
    ...
    url = f'https://api.vercel.com/v1/edge-config/{config["id"]}/item'
    data = {
        'key': key,
        'value': value
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Edge Config write error: {response.status_code} - {response.text}")
            return False
    except Exception as e:
...
def init_edge_config():
    try:
        existing_data = read_edge_config('app_data')
        if existing_data:
            return
        ...
        write_edge_config('app_data', default_data)
...
def get_app_data():
    data = read_edge_config('app_data')
    if not data:
        init_edge_config()
        data = read_edge_config('app_data')
    return data
...
def save_app_data(data):
    return write_edge_config('app_data', data)
